...
|                        | PATCH    | `/api/shopping-items/bulk-status`                         | 複数アイテムのステータス一括更新（購入済みなど） | `shoppingStore` (`shoppingItemApi`)             |
...
| **買い物アイテム詳細** | PUT      | `/api/shopping-items/{itemId}`                            | アイテム基本情報の更新                           | `shoppingStore` (`shoppingItemApi`)             |
